...
\TODO{expand from outline \S2. Brief stubs below.}
...
The signature circuit verifies the issuer's ES256 signature over the payload digest
$e$; the hash circuit recomputes, in-circuit: (a)~the SD-JWT structure and $e$,
(b)~for each presented disclosure, its base64url decoding and membership of its digest
in \texttt{\_sd}, and (c)~the key-binding JWT digest tying the presentation to the
verifier's \texttt{nonce}/\texttt{aud}. Each requested claim is handled in one of two
modes: \emph{disclose}---the value is matched against a public pattern and
revealed---or \emph{assert}---the value stays hidden and only a \emph{predicate} over
it (range, equality, set membership) is enforced. The holder-binding key is verified
through the seam but never revealed.
...
\TODO{expand. Built on longfellow-zk (C++); a shared header is the single source of
truth for the seam, MAC machinery, and circuit cache, with per-format front-ends and
per-feature blocks. Circuits are compiled once and cached (zstd).}
...
$w=(\mathit{cred},\sigma,s,r,A,\pi_{\mathrm{rev}})$. $\mathcal{R}(x,w)=1$ iff:
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt]
...
parameter $\lambda$. Proof sketches are given; full proofs are deferred to the
appendix. \TODO{tighten games to pseudocode for camera-ready.}
...
work relative to mdoc's CBOR---the first concrete quantification of this cost.
...
\TODO{add issuer-side cost model / Table 3.}
...
\TODO{from \S5.4 + \S7.5: static-possession scope (no stateful reputation), ROM
reliance, proof size vs.\ SNARKs, single-machine measurement, moving-target upstream.}

\section{Conclusion}\label{sec:conclusion}
\TODO{1 paragraph: transparent ZK turns unmodified SD-JWT VC and mdoc into anonymous
credentials with an issuer-untraceable nullifier, at practical cost and no trusted
setup.}
