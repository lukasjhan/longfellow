...
% Author visibility: \anonfalse = named (ePrint / camera-ready);
%                     \anontrue  = anonymized (double-blind venue submission).
\newif\ifanon
\anonfalse
...
\else
  \author{Jongho Han\\ Hopae \\ \texttt{lukas@hopae.com}\,$\cdot$\,\texttt{lukas.j.han@gmail.com}}
\fi
\date{}
...
only and at a recurring per-presentation cost. Transparent, no-trusted-setup
zero-knowledge can already prove possession of an \emph{unmodified}, issuer-signed
ECDSA credential with no change to issuers or devices---but \emph{longfellow}, which
introduced this, targets only ISO mdoc and offers no defense against linkable repeat
use. We extend this line in three ways: (i)~the \textbf{first zero-knowledge circuits
for the unmodified IETF SD-JWT VC text format} (JSON/JWS/base64url); (ii)~an
\textbf{issuer-unlinkable per-context blind nullifier}---to our knowledge the first
bound to an issuer-signed standard credential that \emph{never de-anonymizes} its
holder and is \emph{publicly verifiable}, unlike PLUME/Semaphore nullifiers, Privacy
Pass/ARC rate-limit tags, and e-cash double-spend tags; and (iii)~signed-range
non-membership revocation (mechanism due to longfellow) extended to SD-JWT VC. A
shared, format-agnostic seam lets the new nullifier and revocation run \emph{unchanged
over both} SD-JWT VC and ISO mdoc. On longfellow-zk, a presentation proves in under
2\,s and verifies in under 0.85\,s on a commodity desktop
($\approx$1.0\,s\,/\,0.45\,s for mdoc), with no trusted setup; the issuer-untraceable
nullifier costs under 5\% on SD-JWT VC ($\approx$5\% on mdoc) over a plain nullifier.
\end{abstract}

\noindent\textbf{Keywords:} anonymous credentials; SD-JWT VC; ISO mdoc; transparent
zero-knowledge; issuer-unlinkable nullifier; selective disclosure; unlinkability;
revocation; eIDAS.
...
JSON/JWS/base64url/salted-hash structure, sharing a format-agnostic seam with ISO
mdoc (\S\ref{sec:construction}).
...
5\% on SD-JWT VC ($\approx$5\% on mdoc) over a plain nullifier, with no increase in proof size, and signed-range revocation
...
\paragraph{Context authorization (a requirement for unlinkability).} Since
$N=H_{\mathsf{nul}}(s,\mathit{ctx})$ collides exactly when $\mathit{ctx}$ collides,
cross-scope unlinkability is a property of the \emph{ctx discipline}, not of the proof
alone, and we require two things of it. \emph{(i)~Domain separation:} $\mathit{ctx}$
binds the verifier's scope, so distinct scopes yield independent nullifiers.
\emph{(ii)~Authorization:} the wallet accepts a $\mathit{ctx}$ only from a party that
can demonstrate authority over that scope; a $\mathit{ctx}$ presented by an unauthorized
party is treated as an attempt to link and is ignored. Under this discipline a repeat
presentation to the \emph{same verifier under the same scope} deliberately yields the
same $N$---the duplication signal the application relies on (one vote per election, one
account per service)---while no other party can induce that collision. Whether that
signal is persistent or periodic is the application's choice of $\mathit{ctx}$
granularity: an epoch-free $\mathit{ctx}$ deduplicates a scope indefinitely (one account
per service), whereas folding an epoch into $\mathit{ctx}$ confines dedup to one period
and refreshes it thereafter (one vote per election)---so the epoch is an optional
namespace knob, not part of domain separation, which needs only the scope. This
authorization is not new infrastructure: it is the verifier (relying-party)
authentication that OpenID4VP and the EUDI ARF~\cite{eudi-arf} already require---e.g., a
relying-party registration certificate scoping what a verifier may request---so the
wallet binds $\mathit{ctx}$ to that authenticated, registered identity and rejects a
$\mathit{ctx}$ asserted by any other origin.
...
pinned to a verifier-chosen value (an \emph{equality} predicate). A shared bytewise
comparator enforces the \emph{range} checks the format already needs---\texttt{now}$\le$\texttt{exp}
on expiry and $\ell<\mathit{rev\_id}<r$ on revocation; exposing it as a general range or
set-membership predicate over arbitrary attributes is future work. The
...
handle. Both ECDSA verifications---the issuer's and the holder's---are the signature
half measured in Table~\ref{tab:eval}.
...
\paragraph{Why this is not textbook commit-and-prove.} The gadget---commit $s$ at
issuance, prove $N=H_{\mathsf{nul}}(s,\mathit{ctx})$ at showing---is deliberately
simple; the contribution is the regime. Untraceable prior nullifiers avoid an issuer
altogether (PLUME's self-held key, Semaphore's admin-known leaf), making untraceability
vacuous; ours binds to a credential that a real, later-adversarial issuer
signed---its signature covering the commitment---yet still cannot link. Two constraints
make this nontrivial: the commitment must be \emph{blind} (an ordinary issuer-signed
\texttt{\_sd} disclosure the issuer never opens), and the seed must be bound end-to-end
in SHA-256 only---from the issuer's signature, through the payload digest $e$ that the
split-circuit seam carries (\S\ref{sec:seam}), down to the \emph{same $s$ wires} that
open $C$ and derive $N$ (steps~4--5)---with no trusted setup. A single-field Groth16
...
256-bit blind $r$ and forms a hash commitment $C=\mathsf{SHA256}(s\|r)$ (written
abstractly as $\mathsf{Commit}(s;r)$ in \S\ref{sec:security}). The issuer
...
observes $s$ or $r$}. This is the issuer's \emph{existing} SD-JWT issuance, unchanged:
\texttt{pseudonym\_commitment} is an ordinary \texttt{\_sd} disclosure whose value is
holder-supplied---just as the holder-binding \texttt{cnf} key is a holder-supplied
value the issuer already attests. No signing logic, key, signature scheme, or
infrastructure changes; the claim set gains one entry, and the issuer need not even
validate $C$, since a malformed commitment only prevents its own holder from later
opening it.
...
see~\S\ref{sec:related}. Untraceability requires the holder to sample $s$ at full
entropy: a low-entropy seed would let an issuer who knows the holder brute-force $N$,
but---like a malformed $C$---this harms only that holder.
...
size} and reveals neither $\mathit{rev\_id}$ nor which gap was used. The holder's proof
is thus $O(1)$ in the list size, but the CRA signs $O(|\text{revoked}|)$ gaps and
re-signs them each epoch---an issuance-side cost borne by the authority, off the
presentation path.
...
$x=(\mathit{ipk},\mathit{ctx},N,D,\phi,\mathit{rinfo})$ of \S\ref{sec:relation},
...
\emph{instantiated as} $\mathsf{SHA256}(s\|\mathit{ctx\_hash})$ with
$\mathit{ctx\_hash}=\mathsf{SHA256}(\mathit{ctx})$ (\S\ref{sec:blind}), modeled as a
random oracle / PRF keyed by $s$.
...
\end{theorem}
\begin{proof}[Proof sketch] Hybrids: replace the proof by $\mathsf{Sim}(x)$ (ZK), which
hides every witness---including the now credential-specific $\mathit{dpk}$, which is not
in $x$; the only $b$-dependent part of $x$ is $N_b$ (since $D,\phi$ are equal by the
restriction); replace $N_b$ by a uniform value (fresh $\mathit{ctx}^*$, high-entropy
$s_b$), after which the view is independent of $b$. \end{proof}
...
assumption}, unlike Groth16-based schemes. The reduction is stated for the relation
$\mathcal{R}$ and further assumes the circuits faithfully encode $\mathcal{R}$---notably
the base64url/JSON and CBOR parsers---which we check by test vectors and differential
testing against reference parsers, not formal verification (an orthogonal assurance
shared by all circuit-based ZK systems). Issuer-untraceability assumes the issuer
...
and three for the base row. The feature rows thus exercise selective disclosure rather
than an attribute predicate; the in-circuit cost of a date-range predicate is nonetheless
represented, since every row's base already enforces \texttt{now}$\le$\texttt{exp} over the
10-digit expiry with the same comparator an \texttt{age\_over\_N} proof
(\texttt{birth\_date}$\le$\texttt{cutoff}) would reuse.

\begin{table}[t]\centering\small
\caption{Per-feature cost on SD-JWT VC and ISO mdoc (median of 7 runs). The signature
half verifies two ECDSA in every row (issuer over $e$, holder KB-JWT over $e_2$);
revocation adds a third, the CRA span.}
\label{tab:eval}
...
\emph{Revocation} adds constant, list-independent overhead. Every row's signature
circuit already verifies \emph{two} ECDSA---the issuer's over $e$ and the holder's
KB-JWT over $e_2$ (\S\ref{sec:frontend}). Each in-circuit P-256 ECDSA costs
$\approx$1{,}549 signature-circuit inputs (the increment the CRA span adds), so the
3{,}739-input base is two such verifications plus $\approx$640 fixed overhead; a single
ECDSA would leave an implausible $\approx$2{,}190 of overhead. Revocation adds a
\emph{third} ECDSA, growing the circuit to 5{,}288 inputs and the bundle by
...
\subsection{Mobile prover}\label{sec:mobile}
We re-ran the full evaluation on a \textbf{Samsung Galaxy Z Flip5} (Snapdragon 8 Gen~2
for Galaxy, 8\,GB RAM) under Termux with identical circuits. A compact foldable throttles
under sustained load, and a fixed configuration order then inflates the later runs by
$\approx$30\%; Table~\ref{tab:mobile} therefore reports the median of seven rounds with the
order \emph{counterbalanced} (rotated each round), which removes that bias. \emph{Mobile
proving is practical:} every configuration proves in under 1.5\,s and verifies in under
0.7\,s---faster than the 2018 desktop across the board (1.3--1.5$\times$)---and the
SD-JWT-VC-$\approx$2$\times$-mdoc ratio (\S\ref{sec:eval}) holds on-device. \emph{Memory is
platform-independent:} peak resident set is 180--410\,MB, essentially identical to the
desktop (180--199\,MB mdoc, 383--414\,MB SD-JWT VC)---it is the Ligero tableau, not a
platform artifact---about 5\% of the phone's 8\,GB. Per-feature differences on mobile stay
within thermal variance; we isolate feature costs on the desktop (Table~\ref{tab:eval}), and
the base row (three disclosed attributes vs.\ one) is not feature-comparable.

\begin{table}[t]\centering\small
\caption{Mobile prover: Samsung Galaxy Z Flip5 (Snapdragon 8 Gen~2, 8\,GB), median of 7
with configuration order counterbalanced (rotated each round) to remove thermal-ordering
bias. Peak RSS is the proof system's working set and matches the desktop
(Table~\ref{tab:eval}'s hardware) within noise.}
\label{tab:mobile}
\begin{tabular}{@{}llrrrr@{}}
\toprule
Format & Feature & Prove & Verify & Proof & Peak RSS \\
       &         & (ms)  & (ms)   & (KB)  & (MB) \\
\midrule
SD-JWT VC & base (SD + sig)        & 1299 & 593 & 392 & 372 \\
SD-JWT VC & + nullifier            & 1392 & 660 & 393 & 395 \\
SD-JWT VC & + \textbf{blind} null. & 1414 & 663 & 396 & 403 \\
SD-JWT VC & + revocation           & 1442 & 692 & 415 & 397 \\
ISO mdoc  & + nullifier            & \phantom{0}694 & 331 & 344 & 180 \\
ISO mdoc  & + \textbf{blind} null. & \phantom{0}731 & 361 & 347 & 187 \\
ISO mdoc  & + revocation           & \phantom{0}757 & 364 & 361 & 182 \\
\bottomrule
\end{tabular}
\end{table}
...
Measurements span an x86 desktop and a Snapdragon~8~Gen~2 phone (\S\ref{sec:mobile}); a
server-class datapoint remains future work. Proving time scales with the number and size of disclosed attributes; we
...
\textbf{Mombelli}~\cite{mombelli2025zkvc} is the closest prior work to target SD-JWT VC
directly, but---finding in-circuit parsing of a real SD-JWT prohibitive ($>$1.2M
constraints for a 1\,KB credential)---it has the \emph{issuer} re-issue the credential
with extra signed claims: a Poseidon Merkle-tree root over the attributes plus a
\emph{second} ECDSA-secp256k1 signature, replacing SHA-256 and P-256 with
circuit-friendly primitives (evaluated mainly under Groth16/PLONK). This requires
issuer-side modification and new (Poseidon, secp256k1) assumptions, and its
inter-credential linking value \texttt{cred\_bind} is a per-subject string the issuer
knows---the opposite of an issuer-untraceable nullifier. We instead verify the genuine
SHA-256 \texttt{\_sd} digests and the original P-256 signature of an \emph{unmodified}
SD-JWT VC, add no algebraic assumption, and bind an issuer-untraceable nullifier.
...
Mombelli~\cite{mombelli2025zkvc} & Groth16/PLONK & T$^{d}$ & SD-JWT$\to$extended & \cmark & \cmark & \cmark/\cmark & \xmark cred\_bind, issuer-known & --- & \cmark\\
...
\textbf{This work} & \textbf{Ligero+sumcheck} & \textbf{N} & \textbf{SD-JWT VC + mdoc} & \cmark & \pmark$^{c}$ & \textbf{\cmark/\cmark} & \textbf{\cmark issuer-untraceable} & signed-range$^{a}$ & \cmark\\
\bottomrule
\end{tabular}}
\\[2pt]
...
\quad $^{c}$Predicates: equality assertion plus the format's built-in date ranges
(expiry, revocation); general range/set-membership over arbitrary attributes is future work.
\quad $^{d}$Mombelli also evaluates transparent backends; the credential modification
(Poseidon root + second secp256k1 signature) is issuer-side and backend-independent.}
\end{table*}
...
nullifier. Our theorems are asymptotic; concretely we adopt longfellow's v7 Ligero
parameters---132 columns opened at rate $1/7$ over $\mathrm{GF}(2^{128})$, reported at
$\approx$109-bit statistical soundness~\cite{frigo2024longfellow,libzk-draft}---atop
SHA-256 Merkle commitments and Fiat--Shamir in the ROM, and do not re-derive these
bounds. As longfellow is recent, that concrete analysis has a
short cryptanalytic track record---an inherited caveat we state rather than resolve.
...
trusted setup. At this size a proof does not fit a QR code; presentment assumes a
return channel (NFC, BLE, or an HTTPS callback), as mDL already uses---though an HTTPS callback exposes
IP/TLS metadata, a linkage vector outside our cryptographic model that proximity
channels (NFC/BLE) avoid. Reducing proof
size (e.g., via recursion or a succinct wrapper) is future work.
...
surveyed schemes.
\paragraph{Measurement and moving target.} Numbers are from an x86 desktop and a
Snapdragon~8~Gen~2 phone (\S\ref{sec:mobile}); a server-class datapoint remains future
work. longfellow
...
yet which costs only $\approx$5\% over a plain nullifier, closes the gap that batch issuance and
...
\item $\mathcal{A}$ outputs $(x^*,\pi^*)$ and wins iff $\mathsf{Ver}(x^*,\pi^*)=1$ and no $\mathit{cred}\in Q$ is both validly signed and consistent with $x^*$ (disclosed values, predicate, device binding, non-revocation).
\end{enumerate}
...
$w^*=(\mathit{cred}^*,\sigma^*,\sigma_{\mathrm{kb}}^*,\mathit{dpk}^*,\allowbreak
s^*,r^*,A^*,\pi_{\mathrm{rev}}^*)$ with
$\mathcal{R}(x^*,w^*)=1$ up to knowledge error $\kappa$. Clause~1 of $\mathcal{R}$ gives
$\mathsf{SigVer}(\mathit{ipk},\mathit{cred}^*,\sigma^*)=1$, and clause~2 a valid holder
signature $\sigma_{\mathrm{kb}}^*$ under $\mathit{dpk}^*$, the holder-binding key
attested in $\mathit{cred}^*$. Two cases.
...
binding of $\mathsf{COM}$), the device key (clause~2: $\mathit{dpk}^*$ equals the attested
\texttt{cnf}/\texttt{deviceKey} and carries a valid $\sigma_{\mathrm{kb}}^*$), and the
non-revocation fact to $\mathit{cred}^*$; hence
