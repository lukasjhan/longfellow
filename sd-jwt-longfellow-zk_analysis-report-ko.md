...
**AMD Ryzen 7 2700X**(8C/16T, 2018 데스크톱), **싱글스레드**(코어 1개 100%), 회로 캐시 워밍, 3회 평균 벽시계:

| 변형 | 속성 | hash 회로 입력수 | prove | verify | 번들 |
|---|---|---:|---:|---:|---:|
| `sdjwt_split` | 3 | ~195k | ~1.94초 | ~0.79초 | 392 KB |
| `sdjwt_null_split` (nullifier) | 1 | ~189k | ~1.84초 | ~0.71초 | 390 KB |
| `sdjwt_null_blind` (blind nullifier) | 1 | ~189k | ~1.95초 | ~0.81초 | 389 KB |
| `sdjwt_revoc_split` (revocation) | 1 | ~189k | ~1.95초 | ~0.82초 | 405 KB |

- **제시 1회당: prove ≈ 1.8–2.0초, verify ≈ 0.7–0.8초, 증명 ≈ 390–405 KB.**
- **일회성 회로 셋업**(geometry당): 콜드 빌드 ≈ **31초**(GF(2¹²⁸) 해시 회로 컴파일), 이후 zstd 캐시 로드 ≈ **0.5초** — 제시마다 드는 셋업은 이것뿐.
- **해시 회로가 지배적**(입력 ≈189–195k vs ECDSA 서명 회로 3.7k): prove 시간 대부분이 GF(2¹²⁸) 위 SHA + `_sd` 멤버십, ECDSA 서명 회로는 상대적으로 작음.
- 이 2018년 CPU에서 **싱글스레드** — 최신 CPU나 멀티스레드 prover면 상당히 단축(멀티코어가 아직 병렬로 안 쓰임).
- `revoc`이 약간 무거움(CRA 서명 + 4번째 MAC → sig 입력 3739→5288, 번들 +15 KB), prove/verify는 오차 범위.
- 피크 RSS ≈ **390 MB**. 검증자-statement 분리(토큰-프리 검증자)는 측정 가능한 비용 없음 — 호스트측 입력 조립만.
