...
- ✅ **Privacy-preserving revocation (signed-span non-membership)** — `pnpm run demo:revocation`. The issuer commits a `revocation_id` (its `_sd` digest = `rev_id`); a revocation authority signs the open gaps `epoch‖l‖r` between adjacent revoked ids, and the circuit proves `l < rev_id < r` in ZK — a **constant-size** non-revocation proof that reveals neither `rev_id` nor which credential it is. Adds `e_span` as a 4th MAC-linked value. Soundness tested: `REVOKED` / `BADSIG` / `STALE` / `TAMPER` all reject. Report: [`sd-jwt-revocation_analysis-report.md`](../sd-jwt-revocation_analysis-report.md).
