// Nullifier prototype: full SD-JWT-VC ZK proof (Approach C) + a pseudonymous
// NULLIFIER (CI/DI-like) derived from an issuer-committed secret.
//
// On top of the full proof (issuer sig + exp + vct + KB + sd_hash + N×_sd), it
// proves in zero knowledge, given a verifier `context_id`:
//   * `pseudonym_secret` is an _sd claim (issuer-committed, so the holder can't
//     choose it), and
//   * nullifier == SHA256( secret(64 hex bytes) ‖ context_id(CTXLEN, padded) )
// revealing only `nullifier` and `context_id` (the secret stays hidden).
//
// Same (secret, context) -> same nullifier (duplicate/Sybil detection within a
// scope = DI); different context -> unlinkable nullifier (across scopes);
// empty/global context -> CI-like cross-service pseudonym. The message is fully
// bound (secret + context + canonical padding) so the nullifier is deterministic.
...
constexpr const char* kSeed = "sdjwt-null";

// Generous, fixed circuit capacities (matched to sdjwt_split.cc); the host
// checks each token against them and errors clearly if exceeded. NOTE: this is
// the single-Fp256 circuit, so bigger constants cost more here than in the split.
...
struct Inputs {
  ...
  v8 aud_len;
  v8 context_id[CTXLEN];   // verifier-chosen nullifier scope (fixed CTXLEN, padded)
  v256 nullifier;          // public output: SHA(secret ‖ context_id)
  ...
  // private: pseudonym nullifier — the pseudonym_secret disclosure + nullifier SHA
  v8 sec_pre[64 * MAXB];
  v256 sec_ebits;
  SBW sec_sha[MAXB];
  v8 sec_nb;
  LC::bitvec<8> sec_len, sec_shift;
  LC::bitvec<LOGM> sec_sd_idx;
  v8 null_pre[64 * NULLB];     // SHA preimage: secret(64) ‖ context(CTXLEN) ‖ pad
  SBW null_sha[NULLB];
  v8 null_nb;
};
...
void declare_inputs(const LC& L, QuadCircuit<Fp256Base>& Q, Inputs& in,
  ...
  for (size_t i = 0; i < CTXLEN; ++i) in.context_id[i] = L.template vinput<8>();
  ...
  // nullifier: secret disclosure + null SHA
  for (size_t i = 0; i < 64 * MAXB; ++i) in.sec_pre[i] = L.template vinput<8>();
  in.sec_ebits = L.template vinput<256>();
  for (size_t i = 0; i < MAXB; ++i) in.sec_sha[i].input(L);
  in.sec_nb = L.template vinput<8>();
  in.sec_len = L.template vinput<8>();
  in.sec_shift = L.template vinput<8>();
  in.sec_sd_idx = L.template vinput<LOGM>();
  for (size_t i = 0; i < 64 * NULLB; ++i) in.null_pre[i] = L.template vinput<8>();
  for (size_t i = 0; i < NULLB; ++i) in.null_sha[i].input(L);
...
void assert_logic(const LC& L, const Inputs& in) {
  ...
  // ===== pseudonym nullifier (CI/DI-like) =====
  // pseudonym_secret is an issuer-committed _sd claim; nullifier = SHA(secret ‖
  // context_id). The whole SHA preimage is bound (secret + context + canonical
  // padding) and null_nb is fixed, so the nullifier is deterministic per
  // (secret, context) — a holder cannot generate two nullifiers for one scope.
  {
    // 1) membership: SHA(secret_disclosure) ∈ _sd
    sha.assert_message_hash(MAXB, in.sec_nb, in.sec_pre, in.sec_ebits, in.sec_sha);
    v8 entry[43];
    r.shift(in.sec_sd_idx, 43, entry, DECP, dec, zero, 3);
    v8 eout[33];
    b64.base64_rawurl_decode(entry, eout, 43);
    for (size_t j = 0; j < 32; ++j) {
      v8 tb; for (size_t c = 0; c < 8; ++c) tb[c] = in.sec_ebits[8 * (31 - j) + c];
      L.assert1(L.eq(8, eout[j].data(), tb.data()));
    }
    // 2) decode disclosure; anchor `["` then the `","pseudonym_secret","` prefix;
    //    the next SECN chars are the (hidden) secret value.
    v8 dd[MAXDD];
    ...
    static const char PFX[] = "\",\"pseudonym_secret\",\"";  // 22 chars
    constexpr size_t PFXN = sizeof(PFX) - 1;
    for (size_t j = 0; j < PFXN; ++j)
      L.assert1(L.eq(8, S[j].data(), vb(L, (uint8_t)PFX[j]).data()));
    // 3) bind the nullifier SHA preimage = secret(SECN) ‖ context(CTXLEN) ‖ pad
    ...
    // 4) SHA(preimage) == nullifier (public)
...
bool check_capacity(const std::string& compact, const std::vector<std::string>& claims,
  ...
  std::string payload = b64url_decode(jwt.substr(d1 + 1, d2 - d1 - 1));
  ...
  for (const auto& cl : claims) {
    ...
    for (auto& d : discs) if (b64url_decode(d).find(key) != std::string::npos) chosen = d;
    ...
    std::string dj = b64url_decode(chosen);
...
bool fill(Dense<Fp256Base>& W, bool full, const Concrete& c) {
  ...
  // ---- private pseudonym nullifier ----
  std::string sec_disc;
  for (auto& d : discs)
    if (b64url_decode(d).find("\"pseudonym_secret\"") != std::string::npos) sec_disc = d;
  if (sec_disc.empty()) { log(ERROR, "pseudonym_secret disclosure not found"); return false; }
  {
    uint8_t dg[32]; ::SHA256((const uint8_t*)sec_disc.data(), sec_disc.size(), dg);
    std::string entry = b64url_encode(dg, 32);
    size_t sidx = payload.find(entry);
    if (sidx == std::string::npos) { log(ERROR, "secret digest not in _sd"); return false; }
    std::string dj = b64url_decode(sec_disc);
    size_t salt_len = dj.find("\",\"") - 2;
    uint8_t din[64 * MAXB]; FlatSHA256Witness::BlockWitness dbw[MAXB]; uint8_t dnumb = 0;
    FlatSHA256Witness::transform_and_witness_message(
        sec_disc.size(), (const uint8_t*)sec_disc.data(), MAXB, dnumb, din, dbw);
    for (size_t i = 0; i < 64 * MAXB; ++i) push_v8(f, din[i]);
    for (size_t i = 0; i < 256; ++i) f.push_back((dg[31 - i / 8] >> (i % 8)) & 1, 1, p256_base);
    for (size_t i = 0; i < MAXB; ++i) fill_sha(f, enc, dbw[i]);
    push_v8(f, dnumb);
    push_v8(f, (uint8_t)sec_disc.size());
    push_v8(f, (uint8_t)(2 + salt_len));
    f.push_back(sidx, LOGM, p256_base);
    // nullifier preimage = secret(SECN) ‖ context(CTXLEN) ‖ canonical padding
    ...
    for (size_t i = 0; i < NULLB; ++i) fill_sha(f, enc, nbw[i]);
    push_v8(f, nnumb);
  }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  c.context = (argc > 8) ? argv[8] : "context-A";             // verifier-chosen scope

  // compute the pseudonym nullifier (host): SHA( secret(SECN) ‖ context(CTXLEN pad) )
  {
    std::vector<std::string> discs; size_t p = c.compact.find('~') + 1, q;
    while ((q = c.compact.find('~', p)) != std::string::npos) { if (q > p) discs.push_back(c.compact.substr(p, q - p)); p = q + 1; }
    std::string sd;
    for (auto& d : discs) if (b64url_decode(d).find("\"pseudonym_secret\"") != std::string::npos) sd = d;
  ...
  // (secret, context) — i.e. tries to mint a second pseudonym to evade dedup.
  // The circuit binds nullifier == SHA(committed secret ‖ context), so REJECT.
  if (std::getenv("EVIL_NULL")) { c.nullifier[0] ^= 1; printf("  [EVIL_NULL] claiming a forged nullifier for same secret/context\n"); }
  ...
  // Circuit cache: depends on nattr + the capacity geometry (so changing the
  // constants auto-invalidates it). Cache next to the binary.
  ...
  printf("  context_id: \"%s\"\n", c.context.c_str());
  printf("  nullifier : ");
  for (int i = 0; i < 32; ++i) printf("%02x", c.nullifier[i]);
  printf("\n");
  printf("ZK prove/verify (full SD-JWT-VC + pseudonym nullifier)...\n");
  bool ok = run_zk(*C, W, pub);
  printf("  result: %s (%zu attrs + issuer sig + KB + sd_hash + vct + exp + nullifier)\n",
