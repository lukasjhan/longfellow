...
//                                 unchanged), PLUS a nullifier block:
//        - a hidden, issuer-committed `pseudonym_secret` IssuerSignedItem whose
//          SHA is proven to appear in the signed MSO valueDigests (membership),
//        - the 64-byte secret value is extracted in-circuit by a literal CBOR
//          anchor (elementIdentifier "pseudonym_secret" immediately followed by
//          elementValue text(64)) — never revealed,
//        - nullifier = SHA( secret(64) ‖ SHA(context)(32) ), a public output.
//
...
//
// Because @lukas.j.han/mdoc issues `pseudonym_secret` as a 64-hex-char value,
// its IssuerSignedItem is ~174 B (3 SHA blocks) and the value field (66 B) does
// NOT fit MdocHash's public-attribute path (2-block item, v1[64], id+value<=56).
// Hence the nullifier rides a dedicated 3-block item buffer with hidden value,
// exactly mirroring the proven SD-JWT split-nullifier design.
...
// nullifier geometry (mirrors sdjwt_null_split, sized for mdoc's CBOR item).
constexpr size_t SECB = 3;     // pseudonym_secret IssuerSignedItem: <=3 SHA blocks (192B)
constexpr size_t SECN = 64;    // secret value = 64 hex chars
constexpr size_t CTXLEN = 32;  // bind SHA(context), not raw context (length-independent)
constexpr size_t NULLB = 2;    // SHA blocks for SECN+CTXLEN = 96B
// Literal CBOR anchor that uniquely locates the secret value inside the item:
...
static constexpr uint8_t ANCHOR[] = {
    ...
    'i',  'e', 'r', 0x70, 'p', 's', 'e', 'u', 'd', 'o', 'n', 'y', 'm', '_', 's',
    'e',  'c', 'r', 'e', 't', 0x6C, 'e', 'l', 'e', 'm', 'e', 'n', 't', 'V', 'a',
    'l',  'u', 'e', 0x78, 0x40};
constexpr size_t ANCHORN = sizeof(ANCHOR);  // 50
constexpr size_t EXTN = ANCHORN + SECN;     // 114
...
struct NullWit {
  v8 sec_item[64 * SECB];   // tagged IssuerSignedItem (D8 18 58 .. A4 ..)
  SBW sec_sha[SECB];
  v8 sec_nb;
...
void assert_nullifier(const LC& L, FlatSHA& sha, Routing<LC>& r,
  ...
  // (3) Extraction: shift the literal anchor to the front and assert it; the 64
  //     value bytes follow contiguously. The anchor pins the attribute identity
  //     (pseudonym_secret) and the value length (78 40 = text(64)).
  v8 ext[EXTN];
  r.shift(nw.sec_anchor, EXTN, ext, 64 * SECB, nw.sec_item, zz, 3);
  for (size_t j = 0; j < ANCHORN; ++j) eqc(ext[j], ANCHOR[j]);

  // (4) nullifier = SHA( value(64) ‖ SHA(context)(32) ), fully bound + padded so
  //     it is deterministic per (secret, context) — Sybil-resistant.
  for (size_t j = 0; j < SECN; ++j)
    L.assert1(L.eq(8, nw.null_pre[j].data(), ext[ANCHORN + j].data()));
  for (size_t j = 0; j < CTXLEN; ++j)
    L.assert1(L.eq(8, nw.null_pre[SECN + j].data(), ctx[j].data()));
  constexpr size_t M = SECN + CTXLEN;  // 96
  eqc(nw.null_pre[M], 0x80);
  for (size_t j = M + 1; j < 64 * NULLB - 8; ++j) eqc(nw.null_pre[j], 0);
...
int main(int argc, char** argv) {
  ...
  // ---- locate the pseudonym_secret item and build the nullifier witness ----
  const FullAttribute* sec = nullptr;
  for (auto& fa : hw->pm_.attributes_)
    if (fa.id_len == 16 && memcmp(fa.doc + fa.id_ind, "pseudonym_secret", 16) == 0) sec = &fa;
  if (!sec) { printf("ERROR: pseudonym_secret not present in mdoc\n"); return 2; }
  hashc::NullHost nh;
  const uint8_t* item = sec->doc + sec->tag_ind;
  size_t item_len = sec->tag_len;
  if (item_len > 64 * hashc::SECB) { printf("ERROR: secret item %zuB > %zuB\n", item_len, 64 * hashc::SECB); return 2; }
  ...
  if (anchor == std::string::npos) { printf("ERROR: anchor not found in secret item\n"); return 2; }
  nh.anchor_off = anchor;
  nh.mso_v = sec->mso.v;

  // nullifier = SHA( secret(64) ‖ SHA(context)(32) )
  uint8_t ctxh[32]; ::SHA256((const uint8_t*)context.data(), context.size(), ctxh);
  uint8_t nmsg[hashc::SECN + hashc::CTXLEN];
  memcpy(nmsg, item + anchor + hashc::ANCHORN, hashc::SECN);
  ...
  char geo[64]; snprintf(geo, sizeof geo, "%zua-secb%zu-nb%zu", npub, hashc::SECB, hashc::NULLB);
  auto Chash = get_circuit<f_128>(Fs, GF2_128_ID, cdir + "/mdoc-nullh-hash-" + geo + ".bin",
