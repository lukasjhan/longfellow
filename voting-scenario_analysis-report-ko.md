...
**영향** (실질 차이는 이것뿐):
- **프라이버시 — 불일치일 때만 (`==`의 한계).** **일치** 시엔 단언과 공개가 동일: 요구값이 어느 쪽이든 공개 `pattern`에 들어가므로, 성공하면 검증자는 값을 알게 됨. 동등성에서 단언의 *유일한* 우위는 **불일치** 때(실제 값 숨김). "드러내지 않고 증명"의 진짜 이득은 **성공해도** 값보다 적게 새는 술어 — **range**(생년월일 숨기고 `나이≥18`), **집합 멤버십**(`city ∈ {…}`), 구간 — 에서 나오는데, 현재 회로는 이를 **미구현**(동등성만; SD-JWT 기반 보고서 `sd-jwt-longfellow-zk_analysis-report-ko.md` §4 참고).
